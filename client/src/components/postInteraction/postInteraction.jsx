...
const interact=async(id,type)=>{
  const res=await apiRequest.post(`/pin/interact/${id}`,{type})
  return res.data;
}
function PostInteraction({postid}) {
  ...
  const {isPending, error, data} = useQuery({
    ...
    queryFn: () => apiRequest.get(`/pin/interaction-check/${postid}`).then((res) => res.data)
  });
  if (isPending || error) return null;
  return (
    <div className="postInteraction">
      <div className="interactionIcons">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" onClick={() => mutation.mutate({id: postid, type: "like"})} style={{cursor: "pointer"}}>
